...
data_file = seaborn.load_dataset('flights')
...
class FlightsDataset(Dataset):
    def __init__(self, data):
        self.data_file = data
        self.original_dataset = self.data_file.passengers.to_numpy()
...
dataset = FlightsDataset(data_file)
...
for i in range(len(dataset)):
    normalized_passenger, target = dataset[i]
    passenger = normalized_passenger
    passenger = passenger.view(1, 12, 1)
    pred = predict(passenger)
    act_pred = scaler.inverse_transform(pred.reshape(-1, 1))
    preds.append(act_pred.item())

months = range(0, data_file.month.size)
passengers = data_file.passengers

plt.figure(figsize = (15, 5))
plt.title("Passengers Predict", fontsize = 16)
plt.plot(months, passengers, 'b', label = 'original')
